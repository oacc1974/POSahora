...
class TestPDFGeneratorCode:
    ...
    def test_pdf_content_extraction(self):
        """Test that PDF contains all required RIDE sections when extracted"""
        # This test validates that the PDF can be parsed and contains expected content
        # Note: PDF content is compressed, so we validate structure not raw bytes
        response = requests.get(
            f"{BASE_URL}/api/fe/documents/{DOCUMENT_ID_AUTORIZADO}/pdf",
            headers=self.headers
        )
        ...
        assert b'ReportLab' in content, "PDF not generated by ReportLab"
        
        # Verify PDF has content stream (compressed data)
        assert b'/Contents' in content, "Missing content stream"
        assert b'/Font' in content, "Missing font definitions"
        
        print("✓ PDF has valid structure with content stream")
